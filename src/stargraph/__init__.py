...
"""Stargraph — orchestration framework for LLMs, ML models, tools, and CLIPS rules.
...
* :func:`tool` — decorator that registers a callable as a Stargraph tool (FR-33).
...
``stargraph.compare`` (FR-27, JSONPatch ``RunDiff``) and a top-level ``stargraph.simulate``
helper are deliberately *not* re-exported here yet — ``compare`` lands in task 3.34
(``stargraph.replay.compare``) and ``simulate`` is currently a method on ``Graph``
(stub at :meth:`stargraph.graph.Graph.simulate`, full impl in task 3.43). They will
...
# PEP 562 module-level ``__getattr__`` so ``import stargraph`` succeeds without
...
def __getattr__(name: str) -> Any:
    if name in _OPTIONAL_PROVIDERS:
        return getattr(importlib.import_module("stargraph.stores"), _OPTIONAL_PROVIDERS[name])
