...
from stargraph.graph._loop_helpers import (
    _HitInterrupt,
    _InterruptOutcome,
    _lookup_node,
...
# The control-flow signal (:class:`_HitInterrupt`), the timeout-race value
# object (:class:`_InterruptOutcome`), and the pure node/block helpers
# (:func:`_lookup_node`, :func:`_parallel_block_for`, :func:`_merge_branch_results`)
# now live in :mod:`stargraph.graph._loop_helpers`. They are imported here both
# because :func:`execute` (and its async sub-helpers below) reference them AND to
# preserve the established import surface -- :mod:`stargraph.runtime.dispatch`
# and :mod:`stargraph.nodes.interrupt.interrupt_node` import :class:`_HitInterrupt`
# from here, and the test suite imports :func:`_merge_branch_results`.
__all__ = ["execute"]
