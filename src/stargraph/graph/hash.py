...
"""JCS structural hash for stargraph graphs (FR-4).
...
* :func:`structural_hash` — sha256 hex digest over the RFC 8785 (JCS)
  ...
  :class:`~stargraph.ir._models.IRDocument` and the run's rule-pack triples.
...
* :func:`runtime_hash` — sha256 hex digest of ``f"{python_version}\\x00{stargraph_version}"``.
  Pinned separately from the structural hash so that an upgrade of the
  Python interpreter or stargraph distribution invalidates cached runs even
...
from stargraph.ir._models import GotoAction, ParallelAction, RetryAction

if TYPE_CHECKING:
    from stargraph.ir._models import IRDocument, PackMount

__all__ = ["runtime_hash", "structural_hash"]

# RFC 8785 / JSON's safe integer domain (IEEE-754 double-precision exact range).
# rfc8785.dumps raises IntegerDomainError outside this range; stargraph encodes
# such ints as a sentinel-tagged string to keep the hash total + deterministic
# without mis-encoding them as floats.
_JCS_INT_MAX = 2**53 - 1
_JCS_INT_MIN = -(2**53 - 1)
_BIGINT_PREFIX = "__stargraph_bigint__:"
...
def _normalize(value: Any) -> Any:
    ...
    1. **Big-int tagging.** Ints outside the JCS-safe integer domain
       ...
       ``"__stargraph_bigint__:<digits>"``. JSON's float-safe integer range
...
def _pack_requires_component(governance: list[PackMount]) -> list[Any] | None:
    ...
    ``[pack_id, stargraph_facts_version_or_None, api_version_or_None]``
    ...
    for pm in governance:
        if pm.requires is None:
            continue
        triples.append((pm.id, pm.requires.stargraph_facts_version, pm.requires.api_version))
    if not triples:
        return None
...
def _state_schema_signature(state_schema: Any) -> Any:
    ...
    All other inputs raise :class:`~stargraph.errors.IRValidationError` (FR-6
    force-loud). Callers that previously fed raw ``dict[str, str]`` must
    compile via :func:`stargraph.graph.definition._compile_state_schema` first;
...
def _structural_hash_dict(payload: dict[str, Any]) -> str:
    ...
    Steps:
        ...
        3. Tag JCS-out-of-range ints as ``__stargraph_bigint__:<digits>``.
...
def runtime_hash(python_version: str, stargraph_version: str) -> str:
    """Hex-encoded sha256 of ``python_version`` + NUL + ``stargraph_version``.
    ...
    """
    encoded = f"{python_version}\x00{stargraph_version}".encode()
