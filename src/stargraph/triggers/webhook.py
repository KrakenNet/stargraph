...
1. **Read raw body + headers**. ``X-Stargraph-Timestamp`` (Unix epoch seconds as
   ASCII) and ``X-Stargraph-Signature`` (lowercase hex digest) are pulled from
...
_DEFAULT_SIGNATURE_HEADER = "x-stargraph-signature"

#: Default request header carrying the Unix-epoch-seconds timestamp.
_DEFAULT_TIMESTAMP_HEADER = "x-stargraph-timestamp"
...
class WebhookSpec(IRBase):
    ...
    Attributes:
        ...
        timestamp_window_seconds: ±window for the
            ``X-Stargraph-Timestamp`` header. Default 300 (5 min) per
        ...
        signature_header: Header name carrying the HMAC digest.
            Default ``X-Stargraph-Signature``.
        timestamp_header: Header name carrying the timestamp.
            Default ``X-Stargraph-Timestamp``.
    """
    ...
    signature_header: str = "X-Stargraph-Signature"
    timestamp_header: str = "X-Stargraph-Timestamp"
...
class WebhookTrigger:
    ...
    def routes(self) -> list[_Route]:
        ...
        non-serve plugin hosts can still import :mod:`stargraph.triggers.webhook`.
        """
        if not self._specs:
            return []
        # Lazy import to keep the module import-light (mirrors
        # :mod:`stargraph.triggers.cron` and :mod:`stargraph.triggers.manual`).
        ...
        for spec in self._specs:
            router.add_api_route(
                ...
                methods=["POST"],
                name=f"stargraph.triggers.webhook.{spec.trigger_id}",
            )
        return [router]
