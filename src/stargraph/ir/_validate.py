...
Each :class:`stargraph.errors.ValidationError` carries structured context
...
* :func:`check_version` (re-exported from :mod:`._versioning`) flags
  ...
  :data:`STARGRAPH_IR_VERSION` (FR-35, AC-19.2).
"""
...
def _to_stargraph_error(detail: ErrorDetails) -> ValidationError:
    """Map a Pydantic :class:`ErrorDetails` to a Stargraph :class:`ValidationError`."""
...
def validate(ir: dict[str, Any] | str) -> list[ValidationError]:
    ...
    try:
        ...
        return [_to_stargraph_error(d) for d in exc.errors(include_url=True, include_input=True)]
...
def _lint_cypher_in_node_configs(doc: IRDocument) -> list[ValidationError]:
    ...
    :class:`stargraph.stores.cypher.Linter` against the string value.
