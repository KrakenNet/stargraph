...
Turns an :class:`~stargraph.ir.IRDocument` into a flat, UI-renderable graph
...
* ``stargraph inspect`` future "structure" view.
...
in by :func:`stargraph.ir.backfill_rule_node_ids` for 1.0.0 docs), and
...
underlying IR action is still :class:`~stargraph.ir.GotoAction`.
"""
...
from stargraph.ir._backfill import backfill_rule_node_ids
...
class UINode:
    ...
    Attributes:
        id: Stable node id (matches :attr:`stargraph.ir.NodeSpec.id`).
