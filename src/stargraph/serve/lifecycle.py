...
:meth:`stargraph.graph.GraphRun.cancel` / :meth:`stargraph.graph.GraphRun.pause`.
The HTTP routes in :mod:`stargraph.serve.api` (task 1.24) call into
...
NFR-7), audit-emits a :class:`~stargraph.runtime.events.BosunAuditEvent`,
...
* The "live :class:`GraphRun` handle lookup" is intentionally **stubbed**
  ...
  orchestrators raise :class:`StargraphRuntimeError` with an explicit
...
from stargraph.serve.contextvars import (
...
_LOG = logging.getLogger("stargraph.serve.lifecycle")
"""Module logger; mirrors the :mod:`stargraph.bosun.signing` convention so
pytest ``caplog`` fixtures pick up the missing-yaml warning. Stargraph's
...
# ``stargraph.bosun.lifecycle`` pack if the rule pack ships separately;
...
# from :class:`GraphRun`.
_LIFECYCLE_PACK_ID = "stargraph.runtime"
...
def _build_audit_event(
    *,
    ...
    # pack id as source mirrors the stargraph.bosun.audit promotion path.
...
_DEFAULT_CONFIG_DIR = Path.home() / ".config" / "stargraph"
"""Default ``<stargraph-config>`` location -- overridable via ``STARGRAPH_CONFIG_DIR``."""
...
def resolve_config_dir(explicit: Path | None = None) -> Path:
    """Resolve the active ``<stargraph-config>`` directory (design §8.3).

    Precedence (high → low):

    1. The ``explicit`` argument (test-supplied or programmatic override).
    2. ``STARGRAPH_CONFIG_DIR`` environment variable.
    3. ``~/.config/stargraph/`` default.
    ...
    if explicit is not None:
        return explicit
    env = os.environ.get("STARGRAPH_CONFIG_DIR")
    if env:
        return Path(env)
...
def _construct_broker(yaml_path: Path) -> Broker:
    ...
    -- no manual sources/registry/router assembly here (Stargraph uses
...
async def broker_lifespan(
    *,
    ...
    Resolves ``<stargraph-config>`` via :func:`resolve_config_dir`, probes
    for ``<config>/nautilus.yaml``, and -- when present -- builds the
    :class:`Broker` via :meth:`Broker.from_config`. The broker is
    stashed on :data:`stargraph.serve.contextvars._broker_var` for the
    ...
    ``WARNING`` (``stargraph.serve.lifecycle`` logger) and yields without
    setting the var -- consumers see :class:`StargraphRuntimeError` on
    ...
    ----------
    config_dir
        Optional override for the ``<stargraph-config>`` directory. When
        ``None`` the resolution falls back to ``STARGRAPH_CONFIG_DIR`` /
        ``~/.config/stargraph/`` per :func:`resolve_config_dir`.

    Yields
    ------
    None
        The lifespan context body. Typically composed inside the
        FastAPI app lifespan factory in :mod:`stargraph.cli.serve`.
    """
