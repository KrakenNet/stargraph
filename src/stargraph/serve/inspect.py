...
""":mod:`stargraph.serve.inspect` -- read-only timeline + state + fact-diff views (FR-26, FR-27).

The implementation body for the ``stargraph inspect <run_id>`` CLI surface
(design §3.1 ``inspect.py`` row, AC-9.1, AC-9.2). The CLI shim in
:mod:`stargraph.cli.inspect` parses arguments and delegates here; this
...
* **Timeline view** (default) -- one line per checkpointed step with
  ...
  :class:`stargraph.serve.history.RunHistory` ``run_event_offsets`` index
...
if TYPE_CHECKING:
    ...
    from stargraph.checkpoint.protocol import Checkpoint, Checkpointer
