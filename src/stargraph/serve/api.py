...
"""Minimal FastAPI app factory for the Stargraph serve surface (POC).

This module ships :func:`create_app` -- the single entry point that the
``stargraph serve`` CLI subcommand (task 1.28) and the integration tests
...
* **In-memory run registry** -- ``app.state.deps`` is a plain ``dict``
  ...
  read this for the live :class:`~stargraph.graph.GraphRun` handle that
...
* **Scheduler dependency** -- ``app.state.deps["scheduler"]`` is the
  :class:`~stargraph.serve.scheduler.Scheduler` instance the lifespan
...
# Helpers and request/response models now live in the sibling ``_api_helpers``
# / ``_api_models`` modules. They are imported here both because the route
# closures below reference them AND to preserve the established import surface:
# tests + other modules import these private names directly from
# :mod:`stargraph.serve.api` (e.g. ``from stargraph.serve.api import
# _parse_last_event_id, _RunsPage``). ``_LAST_EVENT_ID_RE`` is the one name no
# longer referenced in this module's body -- it is kept as a pure re-export.
from stargraph.serve._api_helpers import (
    _LAST_EVENT_ID_RE,  # noqa: F401  # pyright: ignore[reportUnusedImport]  -- re-export only
    _audit_path_for_run,
...
from stargraph.serve.auth import AuthContext, AuthProvider, BypassAuthProvider
from stargraph.serve.lifecycle import cancel_run, pause_run
from stargraph.serve.ratelimit import (
    DEFAULT_REFILL_PER_MINUTE,
    PerActorBucketRegistry,
)
from stargraph.serve.respond import handle_respond
...
if TYPE_CHECKING:
    ...
    from stargraph.serve.history import RunHistory, RunRecord
    from stargraph.serve.profiles import Profile
    from stargraph.serve.scheduler import Scheduler


# Re-exported for the established import surface: tests + other modules import
# these private helpers/models directly from :mod:`stargraph.serve.api` (e.g.
# ``from stargraph.serve.api import _parse_last_event_id, _RunsPage``). The
# definitions now live in the sibling ``_api_helpers`` / ``_api_models``
# modules; this block keeps the public + test-visible surface unchanged.
...
def require(*capabilities: str) -> Callable[..., Any]:
    ...
    capability check in :mod:`stargraph.serve.lifecycle` /
    :mod:`stargraph.serve.respond`, which only runs when the cleared
...
async def _emit_capability_denied_audit(
    ...
    *,
    ...
    :mod:`stargraph.serve.lifecycle` ``_persist_audit`` resolution).
    ...
    * sink ``write`` raises (audit emission MUST NOT mask the 403).
    ...
    if sink is None:
        return
    # Local import: BosunAuditEvent lives in stargraph.runtime.events; the
    ...
    try:
        ...
        logging.getLogger("stargraph.serve.api").warning(
...
def create_app(
    ...
    """Build the Stargraph serve FastAPI app with the POC route set.

    Parameters
    ----------
    profile:
        The active deployment :class:`~stargraph.serve.profiles.Profile`
    ...
    deps:
        ...
        * ``deps["capabilities"]`` -- optional engine-side
          :class:`~stargraph.security.capabilities.Capabilities` instance
    ...
    lifespan:
        ...
        runs the post-yield block on shutdown. The CLI ``stargraph serve``
    ...
    * 404 for missing runs and 422 for malformed bodies are FastAPI's
    ...
    """
    from stargraph import __version__ as _stargraph_version

    app = FastAPI(
        title="stargraph",
        version=_stargraph_version,
        description=(
            "Stargraph serve API (POC slice -- design §5.1 minimal subset). "
            "5 routes: POST /v1/runs, GET /v1/runs/{run_id}, "
            "POST /v1/runs/{run_id}/cancel, POST /v1/runs/{run_id}/pause, "
    ...
    # (cleared honors a ``stargraph.toml`` ``[serve.cleared].auth_provider``
    ...
    # documented in :mod:`stargraph.serve.ratelimit`). Skips if the caller
    ...
    # ``stargraph.toml: counterfactual.rate_limit_per_min``.
    ...
    async def cancel_run_route(  # pyright: ignore[reportUnusedFunction]
        ...
        except StargraphRuntimeError as exc:
            raise HTTPException(
    ...
    async def stream_run_events(  # pyright: ignore[reportUnusedFunction]
        ...
        :class:`~stargraph.serve.broadcast.EventBroadcaster` registered on
        ``app.state.deps["broadcasters"]`` and forwards each yielded
        :class:`~stargraph.runtime.events.Event` to the WebSocket as a
        ...
        overflows into :class:`~stargraph.errors.BroadcasterOverflow`; the
        ...
        * Per-message-deflate: a uvicorn-level flag (``--ws-per-message-
          deflate=False``); see :mod:`stargraph.cli.serve` for the
