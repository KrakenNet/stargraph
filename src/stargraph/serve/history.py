...
* ``pending_runs`` -- durable scheduler queue rows. Satisfies the
  :class:`stargraph.serve.scheduler.PendingStore` Protocol so the
  :class:`stargraph.serve.scheduler.Scheduler` can be wired with a single
...
* :class:`stargraph.serve.scheduler.Scheduler` accepts an optional
...
* :func:`stargraph.serve.lifecycle.cancel_run` /
  :func:`stargraph.serve.lifecycle.pause_run` accept an optional
...
:class:`stargraph.checkpoint.sqlite.SQLiteCheckpointer`'s internal
``_db``) so single-writer constraints are respected. Migration is
already applied by the Checkpointer's ``bootstrap()`` (registered in
:mod:`stargraph.checkpoint.migrations`); calling :meth:`bootstrap` here
...
from stargraph.serve.scheduler import PendingRun
...
class RunHistory:
    ...
    Implements the :class:`stargraph.serve.scheduler.PendingStore` Protocol
    ...
    async def bootstrap(self) -> None:
        ...
        :mod:`stargraph.checkpoint.migrations._m002_run_history` on its
    ...
    def _build_event_offsets(self) -> None:
        ...
        :class:`stargraph.audit.jsonl.JSONLAuditSink`. Two on-disk shapes
