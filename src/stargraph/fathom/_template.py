...
"""``stargraph_action`` deftemplate definition and idempotent registration (FR-3, AC-7.1).

The deftemplate slots mirror the design.md ``stargraph.fathom`` contract: ``kind``
constrains the action vocabulary to Stargraph's six verbs, with structural slots
...
adapter; this template is the Stargraph-side surface only.
"""
...
__all__ = ["STARGRAPH_ACTION_DEFTEMPLATE", "register_stargraph_action_template"]


STARGRAPH_ACTION_DEFTEMPLATE: str = """\
(deftemplate stargraph_action
...
def register_stargraph_action_template(engine: fathom.Engine) -> None:
    """Register ``stargraph_action`` on ``engine`` exactly once per engine identity.
    ...
    if engine in _registered_engines:
        return
    engine.load_clips_function(STARGRAPH_ACTION_DEFTEMPLATE)
