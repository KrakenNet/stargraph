...
:mod:`stargraph.plugin.hookspecs`). The two-stage discipline from research
...
:class:`stargraph.errors.PluginLoadError` per AC-5.5.
...
:func:`stargraph.tools.tool` decorator returns the original callable with
...
class Tool(Protocol):
    ...
    The :func:`stargraph.tools.tool` decorator returns the original callable
...
class ToolRegistry:
    ...
    def register(self, tool: Tool) -> None:
        ...
        ``context`` dict so log scrapers and CLI ``stargraph inspect`` can
    ...
    def compatible_with(self, graph: Graph) -> list[Tool]:
        ...
        Mirrors :func:`stargraph.graph.loop._check_tool_capabilities` semantics:
