...
Phase-1 reference :class:`stargraph.stores.graph.GraphStore` implementation
...
- ``add_triple(s, p, o, props)`` runs the MERGE through
  :class:`stargraph.stores.cypher.Linter` first, then upserts both
...
:func:`stargraph.stores._common._lock_for`. A module-level
...
from stargraph.errors import StoreError
...
from stargraph.stores.graph import GraphPath, NodeRef, ResultSet
...
class RyuGraphStore:
    ...
    def _ensure_open(self) -> None:
        ...
        # ``stargraph.stores`` (for type-only purposes) can load the module
