...
Phase-1 reference :class:`stargraph.stores.vector.VectorStore`
...
- ``bootstrap()`` opens (or creates) the table and writes the FR-8
  ...
  schema_v)`` into a sidecar ``_stargraph_meta`` table. AsyncTable has no
...
All write paths serialise through :func:`stargraph.stores._common._lock_for`
