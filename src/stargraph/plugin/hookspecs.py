...
"""Hook specifications for the Stargraph plugin system.
...
:mod:`stargraph.plugin.types`. Phase-2 backfill (resolved): plugin authors
...
def register_mcp_adapters() -> list[MCPAdapterSpec]:
    ...
    The serve / engine wiring drives :func:`stargraph.adapters.mcp.bind`
...
class TriggerHookSpec:
    ...
    :func:`stargraph.plugin.loader.build_plugin_manager`), so this class is
    ...
    .. note::
       ...
       :mod:`stargraph.plugin.triggers_dispatcher`. Direct ``pm.hook.<name>()``
...
# (pluggy stashes its config inside ``<project>_spec`` dicts, but Stargraph
