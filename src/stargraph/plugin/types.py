...
"""Shared types for the Stargraph plugin contract.

Centralises the type aliases referenced from
:mod:`stargraph.plugin.hookspecs` so plugin authors get a real contract
...
Capabilities, FathomAdapter) — keeps ``stargraph.plugin`` import-light
...
if TYPE_CHECKING:
    ...
    from stargraph.ir._models import StoreSpec as _IRStoreSpec
    from stargraph.runtime.tool_exec import ToolResult as _RuntimeToolResult
...
class BosunAction(BaseModel):
    ...
    Typed parameter for :func:`stargraph.plugin.hookspecs.authorize_action`.
...
# (``stargraph_startup`` / ``stargraph_shutdown``). Aliased so plugin authors
# can ``from stargraph.plugin.types import PluginManager`` without a direct
...
class ToolCall:
    ...
    Distinct from :class:`stargraph.runtime.events.ToolCallEvent` (which is a
...
# :class:`stargraph.runtime.tool_exec.ToolResult` (output, replayed, tokens).
...
class PackSpec:
    ...
    Distinct from :class:`stargraph.ir._models.PackMount` (which is the
...
class MCPAdapterSpec:
    ...
    into :func:`stargraph.adapters.mcp.bind` at the appropriate lifespan
    ...
    :func:`stargraph.adapters.mcp.bind` accepts: either an
    ...
    :class:`stargraph.adapters.mcp._MCPSessionLike`.
    """
    ...
    :func:`stargraph.adapters.mcp.call_tool`."""
