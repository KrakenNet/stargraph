...
"""``stargraph serve`` subcommand -- POC uvicorn boot (FR-26, AC-7.1, design §3.1).

Boots the Stargraph serve FastAPI app via :func:`stargraph.serve.api.create_app`
under :func:`uvicorn.run`. The active deployment profile is resolved by
:func:`stargraph.serve.profiles.select_profile`; the ``--profile`` flag, when
supplied, overrides the env-var rung by setting ``STARGRAPH_PROFILE`` before
the call (the POC ``select_profile`` honors only the env-var rung -- the
CLI rung deferral is documented in :mod:`stargraph.serve.profiles`).

POC scope (per task 1.28 + VE2-1 fix):

* The ``deps`` dict is populated with a real in-process
  :class:`~stargraph.serve.scheduler.Scheduler` plus empty ``runs`` and
...
from stargraph.errors import ProfileViolationError, StargraphRuntimeError
...
def cmd(
    ...
    db_path: Annotated[
        ...
        typer.Option(
            ...
            help=(
                ...
                "restarts). Phase 3 wires this from stargraph.toml."
            ),
        ),
    ] = None,
    audit_log: Annotated[
    ...
    graph_paths: Annotated[
        ...
        typer.Option(
            ...
            help=(
                ...
                "Phase 3 task 2.30 wires this from stargraph.toml."
            ),
        ),
    ] = None,
    lm_url: Annotated[
    ...
    """Boot the Stargraph serve FastAPI app under uvicorn (POC).

    The ``--profile`` flag is forwarded to
    :func:`stargraph.serve.profiles.select_profile` via the
    ``STARGRAPH_PROFILE`` env var (the POC selector reads only the env-var
    ...
    * ``run_history`` -- a :class:`RunHistory` over the SQLite
      ...
      500ing.
    """
    os.environ["STARGRAPH_PROFILE"] = profile
    ...
    from stargraph.cli.run import _configure_lm  # pyright: ignore[reportPrivateUsage]
    ...
    # surprising). Phase 3 reads ``stargraph.toml: serve.checkpoint.path``
    # via the config loader (task 1.27) and feeds it here.
    if db_path is None:
        tmpdir = Path(tempfile.mkdtemp(prefix="stargraph-serve-"))
    ...
    # Phase 3 task 2.30 reads this from ``stargraph.toml: artifacts.root``.
    ...
    # `stargraph run` CLI so serve + run share the same import-and-wire
