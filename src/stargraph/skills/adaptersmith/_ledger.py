...
"""Adapter smith ledger — the shared :class:`Ledger` bound to ``ADAPTERSMITH_HOME``.

All behavior lives in :mod:`stargraph.skills._smith.ledger`; this module binds it
to the adapter smith's home (default ``.stargraph/adaptersmith``) and exposes the
...
_LEDGER = Ledger(home_env="ADAPTERSMITH_HOME", home_default=".stargraph/adaptersmith")
