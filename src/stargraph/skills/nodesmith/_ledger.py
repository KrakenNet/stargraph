...
"""Node smith ledger — the shared :class:`Ledger` bound to ``NODESMITH_HOME``.

All behavior lives in :mod:`stargraph.skills._smith.ledger`; this module just
binds it to the node smith's home (default ``.stargraph/nodesmith``) and exposes
the methods as module-level functions so existing callers keep importing them
from here.
"""
...
_LEDGER = Ledger(home_env="NODESMITH_HOME", home_default=".stargraph/nodesmith")
