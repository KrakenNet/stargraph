...
Composes :class:`~stargraph.nodes.retrieval.RetrievalNode` (vector + doc only)
...
from stargraph.stores.vector import Hit  # noqa: TC001 -- used as Pydantic field type
...
class RagState(BaseModel):
    ...
    ``retrieved`` carries the fused :class:`~stargraph.stores.vector.Hit`
...
class RagSkill(Skill):
    ...
    def _call_llm(self, query: str, hits: list[Hit]) -> str:
        ...
        installed via :func:`stargraph.adapters.dspy._install_filter`, and
        returns the LM-produced ``answer`` field.

        Surfaces :class:`~stargraph.errors.AdapterFallbackError` rather than
