...
"""Trigger smith ledger — the shared :class:`Ledger` bound to ``TRIGGERSMITH_HOME``.

All behavior lives in :mod:`stargraph.skills._smith.ledger`; this module binds it
to the trigger smith's home (default ``.stargraph/triggersmith``) and exposes the
methods as module-level functions so callers import them from here.
"""
...
_LEDGER = Ledger(home_env="TRIGGERSMITH_HOME", home_default=".stargraph/triggersmith")
