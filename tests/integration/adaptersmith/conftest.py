...
"""Shared fixtures for adaptersmith integration tests."""
...
def _isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:  # pyright: ignore[reportUnusedFunction]
    """Point ADAPTERSMITH_HOME at a throwaway dir so the ledger never touches the repo."""
    monkeypatch.setenv("ADAPTERSMITH_HOME", str(tmp_path / "x"))
