...
async def test_migration_003_adds_error_columns_idempotently(tmp_path: Path) -> None:
    ...
    try:
        ...
        await db.execute("CREATE TABLE runs_history (run_id TEXT PRIMARY KEY, status TEXT)")
        await db.execute("INSERT INTO runs_history (run_id, status) VALUES ('legacy', 'done')")
        await db.commit()
