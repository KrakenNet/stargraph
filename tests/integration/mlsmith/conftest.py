...
"""Shared fixtures for mlsmith integration tests."""
...
def _isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:  # pyright: ignore[reportUnusedFunction]
    """Point MLSMITH_HOME at a throwaway dir so the ledger never touches the repo."""
    monkeypatch.setenv("MLSMITH_HOME", str(tmp_path / "ml"))
