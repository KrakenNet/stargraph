...
"""Shared fixtures for graphsmith integration tests."""
...
def _isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:  # pyright: ignore[reportUnusedFunction]
    """Point GRAPHSMITH_HOME at a throwaway dir so the ledger never touches the repo."""
    monkeypatch.setenv("GRAPHSMITH_HOME", str(tmp_path / "gs"))
