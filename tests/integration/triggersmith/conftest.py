...
"""Shared fixtures for triggersmith integration tests."""
...
def _isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:  # pyright: ignore[reportUnusedFunction]
    """Point TRIGGERSMITH_HOME at a throwaway dir so the ledger never touches the repo."""
    monkeypatch.setenv("TRIGGERSMITH_HOME", str(tmp_path / "ts"))
