...
"""Shared fixtures for skillsmith integration tests."""
...
def _isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:  # pyright: ignore[reportUnusedFunction]
    """Point SKILLSMITH_HOME at a throwaway dir so the ledger never touches the repo."""
    monkeypatch.setenv("SKILLSMITH_HOME", str(tmp_path / "sk"))
