...
"""Shared fixtures for toolsmith integration tests."""
...
def _isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:  # pyright: ignore[reportUnusedFunction]
    """Point TOOLSMITH_HOME at a throwaway dir so the ledger never touches the repo."""
    monkeypatch.setenv("TOOLSMITH_HOME", str(tmp_path / "ts"))
