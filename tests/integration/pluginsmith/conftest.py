...
"""Shared fixtures for pluginsmith integration tests."""
...
def _isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:  # pyright: ignore[reportUnusedFunction]
    """Point PLUGINSMITH_HOME at a throwaway dir so the ledger never touches the repo."""
    monkeypatch.setenv("PLUGINSMITH_HOME", str(tmp_path / "pl"))
