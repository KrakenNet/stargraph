...
"""Shared fixtures for packsmith integration tests."""
...
def _isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:  # pyright: ignore[reportUnusedFunction]
    """Point PACKSMITH_HOME at a throwaway dir so the ledger never touches the repo."""
    monkeypatch.setenv("PACKSMITH_HOME", str(tmp_path / "pk"))
